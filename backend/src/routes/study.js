...
router.post('/summaries', upload.single('file'), async (req, res) => {
    try {
...
// CREATE Quiz
router.post('/quizzes', upload.single('file'), async (req, res) => {
    try {
        ...
        const quiz = await prisma.quiz.create({
            data: {
                ...
                sourceData,
                questions: {
                    create: aiResult.questions.map((q, index) => ({
                        question: q.question,
                        options: q.options,
    ...
    try {
        ...
        const flashcardSet = await prisma.flashcardSet.create({
            data: {
                ...
                sourceData,
                flashcards: {
                    create: aiResult.flashcards.map((card, index) => ({
                        front: card.front,
                        back: card.back,
                        order: index,
                    })),
...
router.get('/flashcards/user/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
